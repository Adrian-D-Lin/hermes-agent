...
_ACTIVE_AUTHORITY_CAPABILITY: ContextVar[Optional[_AuthorityCapabilityEnvelope]] = (
...
def provider_status(db_path: Optional[str] = None) -> ProviderStatus:
    ...
    if provider is None:
        return ProviderStatus(
            ...
            healthy=False,
            name=None,
            reason=("no authority provider is registered for this database path"),
        )
    healthy = _provider_is_available(provider)
    reason = None
    if not healthy:
        reason = (
            "registered authority provider is unavailable, unhealthy, or incompatible"
        )
...
def recompute_ready(
    ...
    *,
    ...
    Returns the number of tasks promoted. By default this owns an IMMEDIATE
    transaction and must be called outside any existing write transaction.
    Private callers may explicitly compose it as a savepoint; that savepoint
    is not durable until its outer transaction commits.
...
def _dispatch_once_locked(
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        ...
        return any(row["assignee"] and _rpe(row["assignee"]) for row in review_rows)
        ...
        try:
            ...
            try:
                ...
                if "board" in params and "worker_session_id" in params:
                    ...
                    pid = _spawn(
                        claimed, str(workspace), worker_session_id=worker_session_id
                    )
                else:
                    pid = _spawn(claimed, str(workspace))
            except (TypeError, ValueError):
...
def _ensure_writegate_importable() -> bool:
    ...
    try:
        ...
        if (
            "writegate" in sys.modules
            or importlib.util.find_spec("writegate") is not None
        ):
            return True
...
def _persist_worker_session_id(
    ...
    with write_txn(conn):
        cur = conn.execute(
            "UPDATE task_runs SET worker_session_id = ? WHERE id = ? AND task_id = ?",
    ...
    * the central WriteGate registry has an *active* binding whose
    ...
    try:
        ...
        if task is None:
            return None
        if profile and (
            task.assignee is None or _normalize_assignee(task.assignee) != profile
        ):
            return None
        if workspace and (
