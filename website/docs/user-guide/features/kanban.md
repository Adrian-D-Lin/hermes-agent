...
- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas. By default (`kanban.auto_decompose: false`), tasks remain there until the configured orchestrator or an authenticated dashboard user handles them. You can manually migrate a card or use **⚗ Decompose** / **✨ Specify** from the card. Set `kanban.auto_decompose: true` only when you deliberately want the dispatcher to auto-run the **decomposer** on each tick. The built-in decomposer uses the `auxiliary.kanban_decomposer` model path, reads your profile roster (with descriptions), and fans a task out into a small graph of child tasks routed to the best-fit specialists.
...
**Manual (default)** — `kanban.auto_decompose: false`. Triage tasks stay in triage until the configured orchestrator handles them or an authenticated dashboard user manually migrates them. A dashboard user can click the **⚗ Decompose** button on a card, run `hermes kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This is the default because it keeps triage handling under explicit control.

**Auto (explicit opt-in)** — set `kanban.auto_decompose: true`. The gateway-embedded dispatcher then runs the **decomposer** on each tick, capped by `kanban.auto_decompose_per_tick` (default 3 tasks per tick) so a bulk-load of triage tasks does not burst-spend the auxiliary LLM. The decomposer uses the built-in decomposition prompt plus the `auxiliary.kanban_decomposer` model path to produce a JSON task graph.
...
| Key | Default | Purpose |
|---|---|---|
| `auto_decompose` | `false` | Set `true` to opt into dispatcher auto-decomposition every tick. It does not gate profile-driven `kanban_create` calls or creator wake turns. |
