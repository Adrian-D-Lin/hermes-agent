...
- 一个 **Kanban** 标签页，每个状态显示一列：`triage`、`todo`、`ready`、`running`、`blocked`、`done`（开启切换时还有 `archived`）。
  - `triage` 是粗略想法的停车列。默认情况下（`kanban.auto_decompose: false`），任务会保留在这里，直到配置的编排器或经过身份验证的仪表盘用户处理它们。你可以手动迁移卡片，或在卡片上使用 **⚗ Decompose** / **✨ Specify**。只有在明确希望调度器每个 tick 自动运行**分解器**时，才设置 `kanban.auto_decompose: true`。
...
**手动（默认）** —— `kanban.auto_decompose: false`。分诊任务保留在分诊中，直到配置的编排器处理它们，或经过身份验证的仪表盘用户手动迁移它们。仪表盘用户可以点击卡片上的 **⚗ Decompose** 按钮，运行 `hermes kanban decompose <id>`（或 `--all`），或从聊天中使用 `/kanban decompose <id>`。

**自动（明确选择启用）** —— 设置 `kanban.auto_decompose: true`。Gateway 内嵌调度器随后会在每个 tick 运行**分解器**，并受 `kanban.auto_decompose_per_tick`（默认每 tick 3 个任务）限制。
...
| 键 | 默认值 | 用途 |
|---|---|---|
| `auto_decompose` | `false` | 设置为 `true` 以选择让调度器每个 tick 自动运行分解器；它不会限制配置文件驱动的 `kanban_create` 或创建者唤醒回合。 |
